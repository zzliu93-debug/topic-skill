...
为 Obsidian 知识库生成按讨论触发度排序的 AI 日报、抖音混合选题与逐题素材包的 Codex Skill。
...
所有内容写入知识库的 `AI资讯` 目录。日报不显示来源和链接，而是以“发生了什么事 / 值得关注什么”呈现合并后的事件，并按最可能引发评论或讨论的顺序排列；素材包仍保留可核验出处。教学题在缺少公开热度证据时标为编辑判断，不伪装成平台热门。
